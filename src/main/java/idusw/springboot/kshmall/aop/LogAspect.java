package idusw.springboot.kshmall.aop;
...
class LogAspect { // @AspectJ 지원 AOP, 횡단 관심 - 공통 기능 (로그, .... ),
    ...
    @Pointcut("execution(* idusw.springboot.kshmall.serivce.*ServiceImpl.loginById*(..))")
...
//        logger.info(message); // vs. System.out.println() : 표준 출력으로 다름
    ...
    @AfterReturning("execution(* idusw.springboot.kshmall.serivce.MemberServiceImpl.*(..))")
